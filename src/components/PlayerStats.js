...
export default class PlayerStats extends React.Component {
  ...
  componentDidUpdate(prevProps) {
    if (this.props.name !== prevProps.name) this.refresh();
  }

  updateStats = async user_id => {
    ...
    let cas_kd =
      resp.seasonal.total_casualkills / resp.seasonal.total_casualdeaths;
    let cas_wr =
      resp.seasonal.total_casualwins / resp.seasonal.total_casualtotal;
    let rank_kd =
      resp.seasonal.total_rankedkills / resp.seasonal.total_rankeddeaths;
    let rank_wr =
      resp.seasonal.total_rankedwins / resp.seasonal.total_rankedtotal;
    ...
    if (this.state.results.length > 0) {
            ...
            {this.state.results.map((user, idx) => {
              return (
                ...
                >
                  ...
                  <img
                    className="profile_pic_small"
                    alt="profile_pic"
                    src={`https://ubisoft-avatars.akamaized.net/${
                      user.user
                    }/default_146_146.png`}
                  />
                </li>
      ...
      return (
        <div className="player_stats">
          <div
            className={
              "player_stats__header " +
              (this.props.team ? "red_team" : "blue_team")
            }
          >
            <h3 className="player_name">{this.state.name}</h3>
            ...
            <div className="centered_row">
              ...
              <div className="centered_col_compact">
                <img
                  ...
                  src={`https://r6tab.com/images/operators/${
                    this.state.fav_atk
                  }.png`}
                />
              </div>
